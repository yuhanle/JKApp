...
class RNParallax extends Component {
  ...
  getTitleOpacity() {
    return this.state.scrollY.interpolate({
      inputRange: [this.getHeaderMaxHeight() - this.getHeaderMinHeight() - 40, this.getHeaderMaxHeight(), this.getHeaderMaxHeight() + this.getHeaderMinHeight()],
      outputRange: [0, 0.8, 1],
      extrapolate: 'clamp',
    });
  }
  ...
  renderHeaderBackground() {
    const { backgroundImage, backgroundColor, renderSubView } = this.props;
    const imageOpacity = this.getImageOpacity();
    const subViewOpacity = this.getSubViewOpacity();
    ...
    return (
      ...
      >
        ...
        {this.renderLinearGradientBackground()}
        <Animated.View style={[styles.subView, {opacity: subViewOpacity}]}>
          {renderSubView()}
        </Animated.View>
      </Animated.View>
    );
